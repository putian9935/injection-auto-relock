...
async def main():       
    ...
    for i in range(len(actions)):
        Label(win, text = actions[i], font =('Helvetica 15 bold')).grid(row = 0, column = i+1, sticky  = NSEW)
        
    for i in range(3):
        Label(win, text = 'Slave%s'%(i+1), font =('Helvetica 15 bold')).grid(row = 1+i, column = 0, sticky = NSEW)
        
    Label(win, text = 'All', font =('Helvetica 15 bold')).grid(row = 4, column = 0, sticky = NSEW)
    ...
    def create_btn(func):
        ...
        btn =  Button(win, text= 'OFF', font = font.Font(size = 14), command = combine_funcs(switch, func))
        return btn
                ...
                for k in range(1,4):
                    if k != x:
                        if (button_dict[y*10+k]['text'] == 'ON'):
                            button_dict[y*10+k].configure(text = 'OFF', bg = defaultbg)
            
                
            button_dict[i*10+j] = create_btn(test)
            button_dict[i*10+j].grid(row = i, column = j, sticky = NSEW, padx=1, pady=1)
            win.grid_columnconfigure(j,weight = 1); win.grid_rowconfigure(i,weight = 1)
    ...
    for i, l in zip(range(1,4), actions):
        ...
        button_dict[40+i] = Button(win, text= l, font = font.Font(size = 14), command = test2)
        button_dict[40+i].grid(row = 4, column = i, sticky = NSEW, padx=1, pady=1)
        win.grid_columnconfigure(i,weight = 1); win.grid_rowconfigure(4,weight = 1)
